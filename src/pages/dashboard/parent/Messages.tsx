            ...
            messages.map((m) => (
              <button key={m.id} onClick={() => handleSelect(m)}
                ...
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-7 h-7 bg-gold/20 text-navy flex items-center justify-center text-xs font-bold shrink-0">
                    {m.from[0]}
                  </div>
                ...
                <div className={`text-xs truncate mt-1 ${!m.read ? "text-navy" : "text-muted-foreground"}`}>{m.subject_preview}</div>
              </button>
            ))
          )}
        </div>
          ...
          {selected ? (
            <>
              <div className="px-6 py-5 border-b border-border">
                <p className="text-sm text-muted-foreground">{selected.from} · {selected.from_role} · {selected.time}</p>
              </div>
